<section class="md:flex items-center justify-evenly w-full py-56 z-0">
    <div class="flex-col items-start justify-start">
        <h1 class="text-8xl font-header">
            Work-life<br>
            <span class="bg-gradient-to-r from-[#1E3864] via-[#7259A9] via-50% to-[#CC7000] inline-block text-transparent bg-clip-text">Balance</span>
        </h1>
        <h2 class="text-2xl font-body py-5 ml-2 opacity-80">Even is an all in one AI-integrated workspace that handles the stress so you can do the rest.</h2>
    </div>
    <div>
        <h1 class="text-6xl">hey</h1>
    </div>
</section>